...
(function() {
  ...
  window.handlePopupSubmit = async function(popupId) {
    ...
    if (window.validateEmail(email)) {
      console.log('✅ Email validation passed - submitting to database');
      ...
      try {
        ...
        console.log('🔄 Sending payload to popup-track:', payload);
        
        const response = await fetch('https://zsmoutzjhqjgjehaituw.supabase.co/functions/v1/popup-track', {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          console.log('✅ Popup event saved to database:', result);
          alert('Thank you! Check your email for the discount code.');
          document.getElementById(\`smartpop-\${popupId}\`)?.remove();
        } else {
          const errorText = await response.text();
          console.error('❌ Failed to save popup event:', response.status, errorText);
          // Still show success to user but log the error
          alert('Thank you! Check your email for the discount code.');
          document.getElementById(\`smartpop-\${popupId}\`)?.remove();
        }
      } catch (error) {
        console.error('❌ Error saving popup event:', error);
        // Still show success to user but log the error
        alert('Thank you! Check your email for the discount code.');
        document.getElementById(\`smartpop-\${popupId}\`)?.remove();
      }
  ...
  function matchesPageTarget(pageTarget) {
    if (!pageTarget || pageTarget === 'all_pages') {
      return true;
    }
    ...
    console.log(\`🎯 Checking page target: \${pageTarget} for path: \${currentPath}\`);
    ...
    switch (pageTarget) {
      ...
      case 'cart_page':
        return currentPath.includes('/cart') || 
               currentPath.includes('/checkout/cart');
      ...
      default:
        console.log(\`⚠️ Unknown page target: \${pageTarget}, showing on all pages\`);
        return true;
    }
  }
  ...
  function checkTriggers() {
    ...
    for (const popup of activePopups) {
      ...
      if (!matchesPageTarget(popup.page_target)) {
        continue;
      }
  ...
  function showPopup(popup) {
    const isAnnouncement = popup.popup_type === 'announcement';
    const isDiscountOffer = popup.popup_type === 'discount_offer';
    
    // Generate discount display if applicable
    let discountDisplay = '';
    if (isDiscountOffer && (popup.discount_code || popup.discount_percent)) {
      discountDisplay = \`
        <div style="background: #e8f5e8; border: 2px solid #4caf50; border-radius: 8px; 
          ...
          \${popup.discount_code ? 
            \`<div style="color: #2e7d32; font-size: 16px;">
              Use code: <strong>\${popup.discount_code}</strong>
            </div>\` : ''}
        </div>
      \`;
    }
    
    // Generate email input if not announcement
    const emailInput = isAnnouncement ? '' : \`
    ...
    // Update button action based on popup type
    const buttonAction = isAnnouncement ? 
      \`document.getElementById('smartpop-\${popup.id}').remove()\` : 
      \`handlePopupSubmit('\${popup.id}')\`;
    ...
    const popupHTML = \`
      ...
      ">
        ...
        ">
          ...
          <p style="margin: 0 0 \${discountDisplay ? '8px' : '24px'} 0; color: #666; font-size: 16px;">
          ...
          <button onclick="\${buttonAction}" 
            ...
            \${popup.button_text || (isAnnouncement ? 'Got It' : 'Get Offer')}
          </button>
        </div>
      </div>
    \`;
